    ...
    <div className="font-sans" id="about-view">
      ...
      <div className="bg-slate-100 py-3 border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-xs text-gray-500 flex items-center space-x-2">
          ...
          <span>&gt;</span>
          <span className="text-gray-800 font-semibold">{t.navAbout}</span>
        </div>
      </div>
      ...
      <section className="relative bg-slate-900 py-24 text-white overflow-hidden text-center">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 space-y-4">
          <span className="text-[#FFC72C] font-bold text-xs uppercase tracking-widest bg-white/10 px-3 py-1 rounded-full backdrop-blur-sm border border-white/10">
            {t.companyFullName} {t.companyShortName}
          </span>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center text-left">
          ...
          <div className="lg:col-span-7 space-y-6">
            <span className="text-[#FF5A00] font-bold text-xs uppercase tracking-widest block">{t.ourStoryTitle}</span>
          ...
          <div className="lg:col-span-5 grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div className="bg-gradient-to-br from-[#FF5A00] to-[#FF8C00] text-white rounded-2xl p-6 shadow-lg flex flex-col justify-between h-48 hover:scale-[1.02] transition-transform">
              ...
              <div>
                <h4 className="font-bold text-lg leading-tight text-white">{t.globalPartnersLabel}</h4>
                <p className="text-xs text-orange-100 mt-1">{t.globalPartnersDesc}</p>
              </div>
            </div>
            ...
            <div className="bg-[#002D62] text-white rounded-2xl p-6 shadow-lg flex flex-col justify-between h-48 hover:scale-[1.02] transition-transform">
              ...
              <div>
                <h4 className="font-bold text-lg leading-tight text-white">{t.establishedYearLabel}</h4>
                <p className="text-xs text-blue-200 mt-1">{t.establishedYearDesc}</p>
              </div>
            </div>
            ...
            <div className="sm:col-span-2 h-44 rounded-2xl overflow-hidden border border-slate-100 relative shadow-inner">
              ...
              <div className="absolute bottom-3 left-4 text-white z-20">
                <span className="text-xs bg-[#FF5A00] px-2 py-0.5 rounded font-bold uppercase tracking-wider">{t.ntsTeamBadge}</span>
                <p className="text-xs text-gray-200 mt-1 font-semibold">{t.ntsTeamDesc}</p>
              </div>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-2 gap-10">
          ...
          <div className="bg-white rounded-2xl p-8 border border-slate-200/60 shadow-sm text-left flex gap-5">
            ...
            <div className="space-y-2">
              <h4 className="text-2xl font-bold text-[#002D62] uppercase tracking-wide">{t.ourVisionTitle}</h4>
              <p className="text-gray-600 text-sm leading-relaxed">
                {language === 'en' 
                  ? 'To become a symbol of efficiency and reliability in regional logistics, leading digital technology adoption to optimize global cargo flows.'
          ...
          <div className="bg-white rounded-2xl p-8 border border-slate-200/60 shadow-sm text-left flex gap-5">
            ...
            <div className="space-y-2">
              <h4 className="text-2xl font-bold text-[#002D62] uppercase tracking-wide">{t.ourMissionTitle}</h4>
              <p className="text-gray-600 text-sm leading-relaxed">
                {language === 'en'
                  ? 'Simplifying international trade by delivering comprehensive, transparent, and sustainable transportation solutions, helping clients focus entirely on core business growth.'
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <span className="text-[#FF5A00] font-bold text-xs uppercase tracking-widest">
            {language === 'en' ? 'FOUNDATION OF GROWTH' : 'Nền tảng phát triển'}
          </span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 mt-12">
            {[
              { title: t.trustTitle, desc: t.trustDesc, icon: 'Shield' },
              { title: t.speedTitle, desc: t.speedDesc, icon: 'Zap' },
              { title: t.innovationTitle, desc: t.innovationDesc, icon: 'Lightbulb' },
              { title: t.responsibilityTitle, desc: t.responsibilityDesc, icon: 'Users' },
            ].map((val, idx) => (
              <div 
                key={idx}
